...
3. Сборка и тестирование датчика с ESP32
4. Реализация алгоритмов расчета пульса и SpO₂.
5. Настройка передачи данных через BLE.
6. Интеграция с существующими BLE-клиентами для визуализации данных
7. Интеграция системы и тестирование.
8. Подготовка отчета и презентации.

### Этапы выполнения (приблизительно)
#### 1–2 неделя — закупка компонентов, проверка датчика MAX30102
![alt text](https://github.com/MollDoy/SmartHealthPulse/blob/1-2-week/Схема.jpg)
#### 3–4 неделя — чтение данных, отладка фильтрации сигнала.
- Настроено подключение и инициализация датчика MAX30102 через I²C.
- Реализована фильтрация сигнала ИК-канала с помощью Moving Average.
- Проверена стабильность показаний, реализован расчёт BPM.
- Подготовлена основа для вычисления SpO₂ на следующем этапе.
